<div class="modal-header bg-primary ">
    <h5 class="modal-title text-white">{{ !$id ? 'Add' : 'Edit'}} non add-on link</h5>
...
<form method="POST" id="savedata" enctype="multipart/form-data">
    <div class="modal-body">
        ...
        <div class="col-md-12 mb-3">
            <label class="form-label">Non add-on procedure <span class="text-danger">*</span></label>
            <select name="non_add_on_id" id="non_add_on_id" class="form-select" required>
                <option value="">—</option>
                @foreach($procedures as $pid => $pname)
                    <option value="{{ $pid }}" @selected(@$data->non_add_on_id == $pid)>{{ $pname }}</option>
                @endforeach
            </select>
        </div>
    </div>
