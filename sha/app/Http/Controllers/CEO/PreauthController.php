...
class PreauthController extends Controller
{
    ...
    public function pastHistory($id){
        ...
        if($preauth_register){
            ...
            $case_profile = $preauth_register->id;
            $hospital_profile = $preauth_register->hospital_id;
            return view('ceo.past-history',compact('preauth_register','pending_since','case_profile','hospital_profile','general_info','family_history','personal_history','authentication_consent','admission_details','preauth_diagnosis','hospital_speciality','procedures','teams','preauth_teams','investigations','preauth_investigations','preauth_investigation_status'));
        }else{
            return redirect()->route('ceo.dashboard')->with('error','Register Not Found');
        }
    }
    public function approvePreauth(Request $request){
        ...
        $preauth_register = PreauthRegister::where('id',$preauth_register_id)->where('status',PreauthRegister::STATUS_MEDICAL_COMMITTEE_APPROVED)->first();
        ...
        if($preauth_register){
            ...
            if($request->preauth_status == 'Approve'){
                if($request->approved_amount > 100000){
                    ...
                    $preauth_procedure->deducted_amount = $preauth_procedure->price-$request->approved_amount;
                    $preauth_procedure->save();
                }
                $preauth_register->ceo_approved_date = Carbon::now();
                $preauth_register->ceo_approved_amount = $request->approved_amount;
                ...
                $preauth_register->status = PreauthRegister::STATUS_CEO_REJECTED;
                $preauth_register->ceo_approved_date = null;
                $preauth_register->preauth_approved_date = null;
                $preauth_register->ceo_approved_amount = null;
                $preauth_register->preauth_approved_amount = null;
                $status = 'Rejected';
                $preauth_procedure->deducted_amount = 0;
                $preauth_procedure->save();
            }elseif($request->preauth_status == 'Query'){
                $preauth_register->status = PreauthRegister::STATUS_CEO_QUERIED;
                $preauth_register->ceo_approved_date = null;
                $preauth_register->preauth_approved_date = null;
                $preauth_register->ceo_approved_amount = null;
                $preauth_register->preauth_approved_amount = null;
            ...
            $preauth_register->ceo_remarks = $request->remarks;
            $preauth_register->ceo_forwarded_by = auth()->user()->id;
            $preauth_register->save();
            $log_data = array(
                'stage' => 'Preauthorization - CEO',
                'type' => 'Preauthorization - '.$status,
                'remarks' => $preauth_register->ceo_remarks,
            );
            if($request->preauth_status == 'Approve'){
    ...
    public function loadpdf(Request $request, $id) {
        ...
        //     return view('ceo._partials.loadpdf',  compact('investigation', 'id', 'preauth_register'));
        // } else{
    ...
    public function verifydocument(Request $request, $id) {
        ...
        if($request->type == 'preauth_query_supporting_doc' || $request->type == 'committee_query_supporting_doc' || $request->type == 'ceo_query_supporting_doc' || $request->type == 'acs_query_supporting_doc' || $request->type == 'born_baby_birth_certificate' || $request->type == 'preauth_query_add_doc' || $request->type == "hospital_declaration_form") {
            ...
            $documentStatus = $preauth_register->ceo_status ? json_decode($preauth_register->ceo_status, true) : [];
            $documentStatus[$request->type] = $request->status;
            $preauth_register->ceo_status = json_encode($documentStatus);
            
            $preauth_register->save();

            $docname = ucwords(str_replace('_', ' ', $request->type));
            $documentsinfo = Helpers::checkDocStepSeen($id,'ceo_status');
            ...
            if($request->id) {
                $investigation = $preauth_register->enhancement_docs()->where('id', $request->id)->first();
                $investigation->ceo_status = $request->status;
                $investigation->save();
