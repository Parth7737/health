...
class PreauthController extends Controller
{
    ...
    public function pastHistory($id){
        ...
        if($preauth_register){
            ...
            $case_profile = $preauth_register->id;
            $hospital_profile = $preauth_register->hospital_id;
            return view('medical-committee.past-history',compact('preauth_register','pending_since','case_profile','hospital_profile','general_info','family_history','personal_history','authentication_consent','admission_details','preauth_diagnosis','hospital_speciality','procedures','teams','preauth_teams','investigations','preauth_investigations','preauth_investigation_status'));
        }else{
            return redirect()->route('medical-committee.dashboard')->with('error','Register Not Found');
        }
    }
    public function approvePreauth(Request $request){
        ...
        $validatedData = $request->validate([
            ...
            'remarks' => 'required',
            'committee_approved_amount' => 'required_if:preauth_status,Approve',
        ]);
        $preauth_register_id = $request->preauth_register_id;
        $preauth_register = PreauthRegister::where('id',$preauth_register_id)->where('status',PreauthRegister::STATUS_MEDICAL_COMMITTEE_PENDING)->first();
        ...
        if($preauth_register){
            ...
            if($request->preauth_status == 'Approve'){
                ...
                $preauth_register->committee_approved_date = Carbon::now();
                $preauth_register->committee_approved_amount = $request->committee_approved_amount;
                $status = 'Approved';
            }elseif($request->preauth_status == 'Reject'){
                $preauth_register->status = PreauthRegister::STATUS_MEDICAL_COMMITTEE_REJECTED;
                $preauth_register->committee_approved_date = null;
                $preauth_register->committee_approved_amount = null;
                $status = 'Rejected';
            }elseif($request->preauth_status == 'Query'){
                $preauth_register->status = PreauthRegister::STATUS_MEDICAL_COMMITTEE_QUERIED;
                $preauth_register->committee_approved_date = null;
                $preauth_register->committee_approved_amount = null;
                $status = 'Queried';
            }
            $preauth_register->committee_remarks = $request->remarks;
            $preauth_register->committee_forwarded_by = auth()->user()->id;
            $preauth_register->save();
            $log_data = array(
                'stage' => 'Preauthorization - MC',
                'type' => 'Preauthorization - '.$status,
                'remarks' => $preauth_register->committee_remarks,
            );
            if($request->preauth_status == 'Approve'){
                Helpers::addCaseLog($preauth_register->id,$log_data,5,$request->committee_approved_amount);
            }else{
    ...
    public function loadpdf(Request $request, $id) {
        ...
        //     return view('medical-committee._partials.loadpdf',  compact('investigation', 'id', 'preauth_register'));
        // } else{
    ...
    public function verifydocument(Request $request, $id) {
        ...
        if($request->type == 'preauth_query_supporting_doc' || $request->type == 'committee_query_supporting_doc' || $request->type == 'ceo_query_supporting_doc' || $request->type == 'acs_query_supporting_doc' || $request->type == 'born_baby_birth_certificate' || $request->type == 'preauth_query_add_doc' || $request->type == "hospital_declaration_form") {
            ...
            $documentStatus = $preauth_register->medical_committee_status ? json_decode($preauth_register->medical_committee_status, true) : [];
            $documentStatus[$request->type] = $request->status;
            $preauth_register->medical_committee_status = json_encode($documentStatus);
            
            $preauth_register->save();

            $docname = ucwords(str_replace('_', ' ', $request->type));
            $documentsinfo = Helpers::checkDocStepSeen($id,'medical_committee_status');
            ...
            if($request->id) {
                $investigation = $preauth_register->enhancement_docs()->where('id', $request->id)->first();
                $investigation->medical_committee_status = $request->status;
                $investigation->save();
