            ...
            <div class=" mt-5 ">
                <form id="establishmentForm">
                            ...
                            <tr>
                                <td>
                                    <div class="form-floating form-floating-outline">
                                        <select class="select2 eerror" id="dec_verify_statussestablish" name="dec_verify_status" @if($isedit) disabled @endif required>
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <button type="button" class="btn btn-primary rounded-0 saveestablishmentForm">Save</button>
                    </div>
                    @endif
                </form>
            </div>
        ...
        <div class="row"> 
            <div class=" mt-5 ">
                <form id="addressForm">
                    <table class="table table-responsive table-bordered">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <div class="form-floating form-floating-outline">
                                        <select class="select2 aerror" @if($isedit) disabled @endif id="dec_verify_statusaddress" name="dec_verify_status" required>
                                            <option value="">Select</option>
                    ...
                    @if(!$isedit)
                    <div class="d-flex justify-content-end mt-3">
                        <button type="button" class="btn btn-primary rounded-0 saveaddressForm">Save</button>
                    </div>
                    @endif
                </form>
            </div>
...
<script>
    $('.saveestablishmentForm').on('click', function() {
        ...
        $.ajax({
            url: '{{route("decverifier.saveEstablishmentReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
    ...
    $('.saveaddressForm').on('click', function() {
        ...
        $.ajax({
            url: '{{route("decverifier.saveAddressReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
