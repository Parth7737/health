        ...
        <div class="row g-5">
            <div class="col-md-6 col-lg-3 ">
                <div class="input-group input-group-merge sperror">
                    ...
                    <button class="input-group-text btnspecialities" onclick="verifyspecialitieshpr();" type="button">Verify</button>
                </div>
            ...
            <div class="col-md-6 col-lg-3">
                <div class="form-floating form-floating-outline sperror">
                    <input type="text" oninput="sanitize(this, 'b');" id="registration_no" name="registration_no" required class="form-control" placeholder="xxxxxx56" />
...
<script>
    ...
    function verifyspecialitieshpr() {
        var healthcare_proffessionals_registry_id = $('#hpr_id').val();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.success) {
                        ...
                        $('#hpr_id').prop('readonly', true);
                        $(".btnspecialities").attr('disabled', true);
                        $(".btnspecialities").html('<i class="tf-icons ri-check-fill text-green"></i>');
    ...
    $('.savespecialities').click(function () {
        ...
        $.ajax({
            url: '{{route("hospital.empanelmentRegistration.saveHumanSpecialities", [$uuid, $hospital_id])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
   ...
   function loadSpecialitiesTable() {
        ...
        var type = "mhr";
        $.ajax({
            url: '{{route("hospital.empanelmentRegistration.loadSpecialitiesTable", [$uuid, $hospital_id])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
   ...
   function deleteSpecialityData(id, hospital_id) {
        ldrshow();    
        fetch('{{route("hospital.empanelmentRegistration.deleteSpecialitiesHR")}}', {
            method: 'POST',
            headers: {
