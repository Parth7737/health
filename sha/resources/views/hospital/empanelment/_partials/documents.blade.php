...
@elseif(@$hospital->status == 'Response Required From Facility' || @$hospital->status == 'Queried')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="col-md-12 mb-4">
                    <div class="row">
                        @php
                            $getWorkData = App\Models\WorkFlowHistory::where('id', $hospital->dec_work_id)->first();
                        @endphp
                ...
                <form id="hospitalReportFormDec" enctype="multipart/form-data">               
                    <div class="row g-5">  
                        ...
                        <div class="col-sm-12">
                            ...
                            <textarea type="text" class="form-control reporterrormesage" id="dec_remarks"  name="dec_remarks" ></textarea>
                        </div>
...
<script>
    function askHospitalID() {
        swal({
            title: "Enter Hospital ID",
            text: "Please enter your existing Hospital ID for verification.",
            content: {
                element: "input",
                attributes: {
            ...
            if (hospitalID === null) {
                swal("Cancelled!", "Hospital ID verification was cancelled.", "error");
            } else if (!hospitalID.trim()) {
                swal("Error!", "Hospital ID is required.", "error").then(() => {
                    askHospitalID();
                });
            } else {
                ldrshow()
                $.ajax({
                    url: '{{route("hospital.checkexisthospital")}}',
                    headers: {
                    ...
                    success: function (response) {
                        ldrhide()
                        if (response.success) {
                            swal("Success!", "Hospital ID verified successfully!", "success")
                                .then(() => {
                                    submitForm(hospitalID);
                                });
                        } else {
                            swal("Invalid ID!", "The entered Hospital ID is incorrect. Please try again.", "error")
                                .then(() => {
                                    askHospitalID(); 
                                });
                        }
    ...
    function submitForm(hospitalID) {
        ldrshow()
        $.ajax({
            url: '{{route("hospital.directSubmit", [$uuid, base64_encode($hospital_id)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: "POST",
            ...
            success: function (response) {
                ldrhide()
                if (response.success) {
                    swal("Success!", "Hospital Submitted!!", "success")
                        .then(() => {
            ...
            error: function () {
                ldrhide();
                errorMessage("Error!", "Something went wrong. Please try again later.", "error");
            },
        });
    }
        ...
        $(".prevsubmit").on('click', function() {
            ...
            swal({
                ...
                if (willDelete) {
                    swal({
                        title: "Confirm Submission?",
                        text: 'You have already Empanelled Hospital?',
                        type: "warning",
                        buttons: {
                            cancel: {
                                visible: true,
                        ...
                        if (willask) {
                            ...
                            ldrshow();
                            $.ajax({
                                url: '{{route("hospital.empanelmentRegistration.hospitalSubmit", [$uuid, $hospital_id])}}',
                                headers: {
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                                },
                                type: 'POST',
