      ...
      <table class="table table-bordered">
         ...
         <tbody
            ...
            @php
			   ...
			   $existData = App\CentralLogics\Helpers::getSingleLicense($hospital_id, $value->id, $v->id);
            @endphp
            <tr>
               <td>
                  {{$k+1}}
               ...
               <td>
                  <div class="file-upload-section lierror">
                     ...
                     <input type="file" class="file-input d-none" required name="document_{{$value->id}}_{{$v->id}}" id="document_{{$value->id}}_{{$v->id}}" />
...
<script>
   ...
   $('.savelicense').click(function () {
      ...
      $.ajax({
         url: '{{route("hospital.empanelmentRegistration.saveLicenses", [$uuid, $hospital_id])}}', // Replace with your server endpoint
         headers: {
         ...
         error: function (xhr) {
            ...
            if (xhr.status === 422) { 
               ...
               for (let field in errors) {
                  $(`[name="${field}"]`).closest('.lierror').after(`<div class="error text-danger licenseerror">${errors[field][0]}</div>`);
               }
            } else {
