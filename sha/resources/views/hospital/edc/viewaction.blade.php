...
@section('title','Dashboard | View EDC Action')
@section('content')
                ...
                <div class="row g-5">
                    <div class="col-sm-3">
                        ...
                        <select name="edc_action" id="edc_action" class="select2 form-select form-select-lg reporterrormesage" data-allow-clear="true" required >
                            ...
                            @foreach(App\CentralLogics\Helpers::getNextStatuses($action->main_status, $action->last_action, 'hospital') as $key => $value)
...
<script>
    ...
    $('.initiateaction').on("click", function() {
        ...
        $.ajax({
            url: '{{route("hospital.updateinitiate-action", [base64_encode($action->id), base64_encode($hospital->uuid)])}}',
            headers: {
            ...
            success: function (response) {
                ...
                if(response.success) {
                    ...
                    window.location = "{{route('hospital.edcindex')}}";
                } else {
