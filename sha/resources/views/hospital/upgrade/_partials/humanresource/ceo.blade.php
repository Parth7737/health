...
<div class="inside-left-info-box @if(!empty($ceo) && $ceo->is_detail_added == 1) success @else pending @endif ceopanel">
    ...
    <form id="ceoform">
        <div class="row g-5">
            ...
            <div class="col-md-6 col-lg-3 ceoerror">
                <div class="form-floating form-floating-outline">
                    <input type="text" id="designation" name="designation" oninput="sanitize(this, 't');" class="form-control" placeholder="CEO" value="{{!empty($ceo) && $ceo->is_detail_added == 1 ? $ceo->designation : ''}}" />
            ...
            <div class="col-md-6 col-lg-3 ceoerror">
                <div class="input-group input-group-merge">
                    <div class="form-floating form-floating-outline">
                        <input type="email" class="form-control" oninput="sanitize(this, 'email');" id="email" placeholder="john.doe" name="email" aria-label="Recipient's username" aria-describedby="email" value="{{!empty($ceo) && $ceo->is_detail_added == 1 ? $ceo->email : ''}}" {{!empty($ceo) && $ceo->is_detail_added == 1 ? 'readonly' : ''}}>
                        <label for="email">Email ID</label>
                    </div>
                    @if(!empty($ceo)) 
                        <button class="input-group-text btnchangeemail " type="button" onclick="changeEmail();">Change</button>
                        <button class="input-group-text btnemail d-none" type="button" onclick="SendOTPOnEmail()">Verify</button>
                    @endif
                </div>
            ...
            <div class="col-md-6 col-lg-3 ceoerror">
                <div class="input-group input-group-merge">
                    ...
                    @if(!empty($ceo))                    
                        <button class="input-group-text btnchangemobile " type="button" onclick="changeMobile();">Change</button>
                        <button class="input-group-text btnmobile_no d-none" type="button" onclick="SendOTPOnMobile();">Verify</button>
                    @endif
                </div>
...
<script>
    ...
    function changeEmail() {
        ...
        $("#saveCEO").attr('disabled', true);
    }

    
    function changeMobile() {
        $('.btnchangemobile').addClass('d-none');
        $('.btnmobile_no').removeClass('d-none');
        $("#mobile_no").val("");
    ...
    $('#saveCEO').click(function () {
        ...
        swal({
            ...
            if(willDelete) {
                ...
                $.ajax({
                    url: '{{route("hospital.upgrade-ceo-details", [$uuid, $hospital_id])}}', // Replace with your server endpoint
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
                    type: 'POST',
