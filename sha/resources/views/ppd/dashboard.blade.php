        ...
        <div class="row">
            <div class="col-md-12">
                <div class="d-flex justify-content-end mb-3">
                    <a href="{{ route('ppd.case-search') }}" class="btn btn-outline-primary me-2"><i class="ri-user-search-line"></i> Case Search</a>
                </div>
            </div>
        </div>
        <div class="row g-6">
                ...
                <div class="card card-border-shadow-primary h-100">
                    <div class="card-body" onclick="getStatusData('{{ PreauthRegister::STATUS_PREAUTH_APPROVED }}')">
                        <div class="d-flex align-items-center mb-2">
                            ...
                            @php $conditions = array('preauth_approve_reject_query_by'=>auth()->user()->id); 
                                $conditionString = collect($conditions)
                                    ->map(fn($v, $k) => "{$k}:{$v}")
                                    ->implode('|');
                            @endphp
                            <small class="mb-0 pt-5 ms-3">{{ Helpers::getTotalAmount(PreauthRegister::STATUS_PREAUTH_APPROVED,$conditions,1,'preauth_amount_without_deduction') }}</small>
                        </div>
                        <h6 class="mb-0 fw-normal">Preauthorization Approved</h6>
                        <a href="{{route('getDownloadReport', [PreauthRegister::STATUS_PREAUTH_APPROVED, 1, base64_encode($conditionString)])}}" class="float-end"><i class="ri-download-line"></i></a>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="card card-border-shadow-danger h-100">
                    <div class="card-body" onclick="getStatusData('{{ PreauthRegister::STATUS_PREAUTH_REJECTED }}')">
                        <div class="d-flex align-items-center mb-2">
                            ...
                            @php 
                                $conditions = array('preauth_approve_reject_query_by'=>auth()->user()->id); 
                                $conditionString = collect($conditions)
                                    ->map(fn($v, $k) => "{$k}:{$v}")
                                    ->implode('|');
                            @endphp
                            <small class="mb-0 pt-5 ms-3">{{ Helpers::getTotalAmount(PreauthRegister::STATUS_PREAUTH_REJECTED,$conditions,0,'preauth_initiated_amount') }}</small>
                        </div>
                        <h6 class="mb-0 fw-normal">Preauthorization Rejected</h6>
                        <a href="{{route('getDownloadReport', [PreauthRegister::STATUS_PREAUTH_REJECTED, 0, base64_encode($conditionString)])}}" class="float-end"><i class="ri-download-line"></i></a>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                        ...
                        <div class="d-flex align-items-center mb-2">
                            ...
                            @php 
                                ...
                                $conditionString = collect($conditions)
                                    ...
                                    ->implode('|');
                            @endphp
                            <small class="mb-0 pt-5 ms-3">{{ Helpers::getTotalAmount(PreauthRegister::STATUS_PREAUTH_QUERIED,$conditions,0,'preauth_initiated_amount') }}</small>
                        </div>
                        <h6 class="mb-0 fw-normal">Preauthorization Queried</h6>
                        <a href="{{route('getDownloadReport', [PreauthRegister::STATUS_PREAUTH_QUERIED, 0, base64_encode($conditionString)])}}" class="float-end"><i class="ri-download-line"></i></a>
                    </div>
                </div>
            </div>
            <!--/ Card Border Shadow -->
                    ...
                    <div class="card-body p-0 demo-vertical-spacing demo-only-element">
                        <div class="form-floating form-floating-outline">
                            <select class="form-select select2" id="status"
                                ...
                                <option value="{{PreauthRegister::STATUS_PREAUTH_PENDING}}" selected>Preauthorization Pending</option>
                                ...
                                <option value="{{PreauthRegister::STATUS_PREAUTH_QUERIED}}">Preauthorization Queried</option>
                            </select>
...
@push('scripts')
<script>
    function generate_dt(url = "{{ route('ppd.dashboard-users') }}") {
