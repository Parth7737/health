...
@php 
    ...
    if($procedure->preauth_status == 'Approved' || $procedure->preauth_status == 'Forwarded To Medical Committee' || $procedure->preauth_implant_status == 'Approved' || in_array($procedure->preauth_register->status,$status_arr)){
        $is_apply=1;
    }else{
        $is_apply=0;
    }
    if($is_apply){
        ...
        if($procedure->preauth_implant_status == 'Approved' || in_array($procedure->preauth_register->status,$status_arr)){
