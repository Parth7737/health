        ...
        <table class="table">
            ...
            @foreach($preauth_register->investigations as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    ...
                    <td style="padding:0;">
                        <a href="javascript:;" onclick="clickondocument('{{$value->id}}');" data-key="{{$key}}" class=" document{{$key}} doc-{{$value->id}}">{{@$value->investigation->name}}
                        </a>
                    </td>
                </tr>
            @endforeach
...
<script>
   ...
   var documentsQueue = [];
    ...
    function clickondocument(id, type = '') {
        ...
        $.ajax({
            url: '{{route("ppd.loadpdf", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id' : id,
    ...
    function verifyDocument(status, id, statustype = '') {
        $(".loader-overlay").show();
        $.ajax({
            url: '{{route("ppd.verifydocument", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id' : id,
