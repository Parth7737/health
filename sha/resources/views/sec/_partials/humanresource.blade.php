                        ...
                        <table class="table table-responsive table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select {{$disabled}} class="select2 ceoerror" id="dec_statusceo" name="dec_status" required>
                                                <option value="">Select</option>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select class="select2 ceoerror" id="sec_statusceo" name="sec_status" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$hospital->ceo->sec_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$hospital->ceo->sec_status == "InValid") selected @endif >InValid</option>
                                            </select>
                                        </div>
                                    </td>
                                    <td>
                            ...
                            <tbody class="table-border-bottom-0 procedure-body">
                                @foreach($hospital->humanResources()->where('type', 'mhr')->get() as $key => $value)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-floating form-floating-outline">
                                                <select class="select2 mhrerror" id="sec_statushumanresource_{{@$value->id}}" name="sec_status_{{@$value->id}}" required>
                                                    <option value="">Select</option>
                        ...
                        <tbody class="table-border-bottom-0 procedure-body">
                            @foreach($hospital->humanResources()->where('type', 'sshr')->get() as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select class="select2 sshrerror" id="sec_statussupportservice_{{@$value->id}}" name="sec_status_{{@$value->id}}" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$value->sec_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$value->sec_status == "InValid") selected @endif >InValid</option>
                                            </select>
                        ...
                        <tbody class="table-border-bottom-0 procedure-body">
                            @foreach($hospital->hospitalTeam as $key => $value)
                                <tr id="hrrow{{$value->id}}">
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select class="select2 specerror" id="sec_statusspeciality_{{@$value->id}}" name="sec_status_{{@$value->id}}" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$value->sec_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$value->sec_status == "InValid") selected @endif >InValid</option>
                                            </select>
...
<script>
    $('.saveceoForm').on('click', function() {        
        ...
        $.ajax({
            url: '{{route("sec.saveCEOReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
    ...
    $('.savemhrForm').on('click', function() {        
        ...
        $.ajax({
            url: '{{route("sec.saveMHRReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
    ...
    $('.savesshrForm').on('click', function() {        
        ...
        $.ajax({
            url: '{{route("sec.saveSSHRReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
    ...
    $('.savespecForm').on('click', function() {
        ...
        $.ajax({
            url: '{{route("sec.saveSPECReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
