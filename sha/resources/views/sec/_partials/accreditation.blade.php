                        ...
                        <tbody class="table-border-bottom-0 procedure-body">
                            <tr>
                                ...
                                <td style="text-wrap: auto;">
                                    <b>Status:</b>{{@$hospital->hospitalAccreditation->dec_verify_status}}</br>
                                    <b>Remark:</b>{{@$hospital->hospitalAccreditation->dec_verify_remark}}
                                </td> 
                                <td style="text-wrap: auto;">
                                    <b>Status:</b>{{@$hospital->hospitalAccreditation->dec_status}}</br>
                                    <b>Remark:</b>{{@$hospital->hospitalAccreditation->dec_remark}}
                                </td>                                 
                                <td>
                                    <div class="form-floating form-floating-outline">
                                        <select class="select2 accreerror" id="sec_status{{@$hospital->hospitalAccreditation->id}}accr" name="sec_status_{{@$hospital->hospitalAccreditation->id}}" required>
                                            <option value="">Select</option>
                                            <option value="Valid" @if(@$hospital->hospitalAccreditation->sec_status == "Valid") selected @endif >Valid</option>
                                            <option value="InValid" @if(@$hospital->hospitalAccreditation->sec_status == "InValid") selected @endif >InValid</option>
                                        </select>
                                    </div>
                                </td>
                                <td>
                                    <div class="form-floating form-floating-outline">
                                        <input type="text"
                                            id="sec_remark_{{@$hospital->hospitalAccreditation->id}}"
                                            name="sec_remark_{{@$hospital->hospitalAccreditation->id}}"
                                            value="{{@$hospital->hospitalAccreditation->sec_remark}}"
...
<script>
    ...
    $('.saveaccreditationform').on('click', function() {
        ...
        $.ajax({
            url: '{{route("sec.saveAccreditationReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
