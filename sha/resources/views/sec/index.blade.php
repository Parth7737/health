...
<div class="container-xxl flex-grow-1 container-p-y">
   <div class="bg-white rounded-3 box-shadow p-5">
                ...
                <div class="card-body">
                    <div class="row row-cols-5">                     
                        ...
                        <div class="col-md-6 col-lg-3">
                            <div
                                class="form-floating form-floating-outline mb-3">
                                @php $HospitalDistrict = App\Models\HospitalDistrict::where('state_id', $state_id)->get(); @endphp
                                <select class="form-select select2" id="district" name="district">
                                    <option value=""></option>
                                    @foreach($HospitalDistrict as $scheme_type)
                                        <option value="{{ $scheme_type->id }}">{{ $scheme_type->name }}</option>
                                    @endforeach
                                </select>
         ...
         <!-- Chart -->
         <div class="row g-6 mb-5">
            <div class="col-12 col-xxl-4 col-md-4 g-6">
...
<script>
   ...
   function fetchChartData(type = 'empanelled') {
      ...
      formData.append('district', district);
      $.ajax({
         url: '{{ route("sec.hospitaltypechart") }}',
         headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
         },
         type: 'POST',
   ...
   function loadspecialitieschart() {
      ...
      formData.append('district', district);
      $.ajax({
         url: '{{ route("sec.specialiitieschart") }}', // Adjust route name as needed
         headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
         },
         type: 'POST',
