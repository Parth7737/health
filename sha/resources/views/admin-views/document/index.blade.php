@extends('layouts.admin.app',['main_li'=>'Investigations','sub_li'=>''])
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center">
                    ...
                    <button class="btn btn-success btn-round ms-auto me-2" id="openImportModal" data-bs-toggle="modal" data-bs-target="#importInvestigationModal">
                        <i class="fa fa-upload"></i> Import Investigations
                    </button>
                    <a href="{{ asset('public/format/csv/master-investigations.csv') }}" download class="btn btn-primary btn-round ms-auto">
                        <i class="fa fa-download"></i> Download CSV Format
                    </a>
                    <button class="btn btn-primary btn-round ms-auto" id="addDocumentBtn" data-bs-toggle="modal" data-bs-target="#addDocumentModal"><i class="fa fa-plus"></i>Add Investigation</button>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        @foreach($documents as $document)
                            <tr>
                                ...
                                <td>{{ $document->name }}</td>
                                <td>{{ @$document->schemeType->name }}</td>
                                <td>{{ $document->code }}</td>
                                <td>
                                    <div class="btn--container justify-content-center">
                                        <a class="btn action-btn btn-xs btn--primary btn-outline-primary" onclick="editData('{{ $document->id }}')" title="Edit Investigation"><i class="fa fa-edit"></i>
                                        </a>
                                        <a class="btn  action-btn btn-xs btn--danger btn-outline-danger" href="javascript:"
                                            onclick="form_alert('document-{{$document->id}}','Want to delete this document ?')" title="Delete Investigation"><i class="fa fa-trash"></i>
                                        </a>
                                        <form action="{{route('admin.document.destroy',[$document->id])}}"
                                                method="post" id="document-{{$document->id}}">
                                            @csrf @method('delete')
                                        </form>
                                    </div>
                                </td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addDocumentModalLabel">Add/Edit Investigation</h5>
...
<div class="modal fade" id="importInvestigationModal" tabindex="-1" role="dialog" aria-labelledby="importInvestigationModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="importInvestigationModalLabel">Import Investigations</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.document.import') }}" method="POST" enctype="multipart/form-data" id="import_procedures_form">
                @csrf
                <div class="modal-body">
                    <!-- File Upload -->
                    <div class="form-group">
...
<script>
    ...
    $('#add_edit_form').on('submit', function (e) {
        ...
        if ($("#edit_id").val() == '') {
            ...
            url = `{{ route('admin.document.update', ':id') }}`.replace(':id', $("#edit_id").val());
