@extends('layouts.admin.app',['main_li'=>'Addon Procedures','sub_li'=>''])
@section('title','Addon Procedures')
@section('content')
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center">
                    <h4 class="card-title">Addon Procedures</h4>
                    ...
                    <a href="{{ asset('public/format/csv/master-addon.csv') }}" download class="btn btn-primary btn-round ms-auto" ><i class="fa fa-download"></i> Download Format</a>
                    <button class="btn btn-primary btn-round ms-auto" id="addAddonProcedure" data-bs-toggle="modal" data-bs-target="#addAddonModal"><i class="fa fa-plus"></i>Add Addon Procedure</button>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        @foreach($procedures as $addon_procedure)
                            <tr>
                                ...
                                <td>{{ @$addon_procedure->procedure->procedure_code_2 }}</td>
                                <td>{{ @$addon_procedure->addon_procedure->procedure_code_2 }}</td>
                                <td>
                                    <div class="btn--container justify-content-center">
                                        <a class="btn action-btn btn-xs btn--primary btn-outline-primary" onclick="editData('{{ $addon_procedure->id }}')" title="Edit Addon Procedure"><i class="fa fa-edit"></i>
                                        </a>
                                        <a class="btn  action-btn btn-xs btn--danger btn-outline-danger" href="javascript:"
                                            onclick="form_alert('addon-{{$addon_procedure->id}}','Want to delete this Addon Procedure ?')" title="Delete Addon Procedure"><i class="fa fa-trash"></i>
                                        </a>
                                        <form action="{{route('admin.addon.destroy',[$addon_procedure->id])}}"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addAddonModalLabel">Add/Edit Addon Procedure</h5>
    ...
    <form action="javascript:" method="post" id="add_edit_form"enctype="multipart/form-data">
        ...
        <div class="modal-body">
            <div class="form-group">
                <label for="email2">Regular Addon Procedure<span class="text-danger">*</span></label>
...
<div class="modal fade" id="importAddOnModal" tabindex="-1" role="dialog" aria-labelledby="importAddOnModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.addon.import') }}" method="POST" enctype="multipart/form-data" id="import_form">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
...
<script>
    ...
    $("#addAddonProcedure").on("click",function(){
    ...
    $('#add_edit_form').on('submit', function (e) {
        ...
        if ($("#edit_id").val() == '') {
            url = "{{ route('admin.addon.store') }}";
            type = "POST";
        } else {
            url = `{{ route('admin.addon.update', ':id') }}`.replace(':id', $("#edit_id").val());
    ...
    function editData(id) {
        const url = `{{ route('admin.addon.show', ':id') }}`.replace(':id', id);
        $.post({
            url: url,
            type: "get",
            beforeSend: function () {
