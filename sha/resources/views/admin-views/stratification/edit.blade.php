@extends('layouts.admin.app',['main_li'=>'Stratifications','sub_li'=>'Edit Stratification'])
@section('title','Edit Stratification')
@section('content')
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Edit Stratification</h4>
            </div>
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="stratification_category_id" id="stratification_category_id" >
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ $stratification->stratification_category_id == $category->id?'selected':'' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="name" id="name" value="{{ $stratification->name }}">
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="rule" id="rule">
                                    <?php foreach (range('A', 'Z') as $letter): ?>
                                        <option value="<?= strtolower($letter) ?>" {{ $stratification->rule == strtolower($letter)?'selected':'' }}><?= $letter ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="code" id="code" value="{{ $stratification->code }}">
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="form-group">
                                <label for="">Stratification Code2<span class="text-danger"> *</span></label>
                                <input type="text" class="form-control" name="code2" id="code2" readonly value="{{ $stratification->code2 }}">
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="procedure_id[]" multiple id="procedure_id" >
                                    ...
                                    @foreach($procedures as $procedure)
                                        <option value="{{ $procedure->id }}" {{ in_array($procedure->id,explode(",",$stratification->procedure_id))?'selected':'' }}>{{ $procedure->procedure_code_2 }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" name="price" id="price" value="{{ $stratification->price }}">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-action">
                    <button class="btn btn-success" id="update-stratification-btn">Save</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    $("#update-stratification-btn").on("click",function(){
        var formData = new FormData($('#stratificationForm')[0]);
        
        $('.error').remove();
        $.ajax({
            url: '{{route("admin.stratification.update",[$stratification->id])}}',
            headers: {
