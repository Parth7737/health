@extends('layouts.admin.app',['main_li'=>'Audit Sub Category','sub_li'=>''])
@section('title','Audit Sub Category')
@section('content')
<div class="row">
    <div class="col-md-12">
        <div class="card">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn--container justify-content-center">
                                        <a class="btn action-btn btn--primary btn-outline-primary btn-xs" onclick="editData('{{ $category->id }}')" title="Edit Sub category"><i class="fa fa-edit"></i></a>
                                        <a class="btn action-btn btn--danger btn-outline-danger btn-xs" href="javascript:" onclick="form_alert('category-{{$category->id}}','Want to delete this audit sub category?')" title="Delete Sub Category"><i class="fa fa-trash"></i></a>
                                        <form action="{{ route('admin.audit-sub-category.destroy', [$category->id]) }}" method="post" id="category-{{$category->id}}">
                                            @csrf @method('delete')
                                        </form>
                                    </div>
                                </td>
...
<script>
    ...
    $('#add_edit_form').on('submit', function (e) {
        ...
        if ($("#edit_id").val() === '') {
            url = "{{ route('admin.audit-sub-category.store') }}";
            type = "POST";
        } else {
            url = `{{ route('admin.audit-sub-category.update', ':id') }}`.replace(':id', $("#edit_id").val());
            type = "POST";
    ...
    function editData(id) {
        const url = `{{ route('admin.audit-sub-category.show', ':id') }}`.replace(':id', id);
