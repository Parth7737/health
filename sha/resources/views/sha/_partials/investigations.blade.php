@php 
    $status = json_decode($preauth_register->sha_status, true);
@endphp
<style>
    .violate {
        color: #757bff;
    ...
    <div class="col-md-3">
        <table class="table">
            ...
            @foreach($preauth_register->investigations as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="status{{$value->id}}">@if($value->sha_status != "") @if($value->sha_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
            ...
            @if($preauth_register->committee_query_supporting_doc)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    ...
                    <td style="padding:0;">
                        <a href="javascript:;" onclick="clickondocument('', 'committee_query_supporting_doc');" class=" document-committee_query_supporting_doc">Preauth Committee Query Supporting Other Document
                        </a>
                    </td>
                </tr>
            @endif
            @if($preauth_register->ceo_query_supporting_doc)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    ...
                    <td style="padding:0;">
                        <a href="javascript:;" onclick="clickondocument('', 'ceo_query_supporting_doc');" class=" document-ceo_query_supporting_doc">Preauth CEO Query Supporting Other Document
                        </a>
                    </td>
                </tr>
            @endif
            @if($preauth_register->acs_query_supporting_doc)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    ...
                    <td style="padding:0;">
                        <a href="javascript:;" onclick="clickondocument('', 'acs_query_supporting_doc');" class=" document-acs_query_supporting_doc">Preauth ACS/Chairman Query Supporting Other Document
                        </a>
                    </td>
                </tr>
            @endif
            @foreach($preauth_register->enhancement_docs as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="enhancement-status{{$value->id}}" >
                        @if($value->sha_status != "") @if($value->sha_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i> @endif
                    </td>
                    <td style="padding: 0;">
            ...
            @if($preauth_register->death_certificate)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    ...
                    <td style="padding:0;">
                        <a href="javascript:;" onclick="clickondocument('', 'death_certificate');" class=" documentt document-death_certificate">Death Certificate
                        </a>
                    </td>
                </tr>
            @endif
            ...
            @foreach($preauth_register->claim_investigations as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="status{{$value->id}}claim">@if($value->sha_status != "") @if($value->sha_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
...
var documentsQueue = [];
    ...
    function clickondocument(id = '', type = '') {
        ...
        $.ajax({
            url: '{{route("sha.loadpdf", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id': id,
    ...
    function verifyDocument(status, id, statustype = '') {
        if(statustype == 'hospital_bill') {
            if($("#bs-rangepicker-third").val() == '') {
                errorMessage('Please Fill Out bill Date');
                return false;
            }            
        }
        if(statustype == 'discharge_summary') {
            if($("#bs-rangepicker-third").val() == '' || $("#bs-rangepicker-forth").val() == '') {
                errorMessage('Please Fill Out Both Date');
                return false;
            }     
        }
        $(".loader-overlay").show();
        $.ajax({
            url: '{{route("sha.verifydocument", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id' : id,
