@php 
    $status = json_decode($preauth_register->cex_status, true);
@endphp
<style>
    .violate {
        color: #757bff;
    ...
    <div class="col-md-3">
        <table class="table">
            ...
            @foreach($preauth_register->investigations as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="status{{$value->id}}">@if($value->cex_status != "") @if($value->cex_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
            ...
            @foreach($preauth_register->enhancement_docs as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="enhancement-status{{$value->id}}">@if($value->cex_status != "") @if($value->cex_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
            ...
            @foreach($preauth_register->claim_investigations as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="status{{$value->id}}claim">@if($value->cex_status != "") @if($value->cex_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
...
<script>
    ...
    function clickondocument(id = '', type = '') {
        ...
        $.ajax({
            url: '{{route("cex.loadpdf", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id': id,
    ...
    function verifyDocument(status, id, statustype = '') {
        ...
        $(".loader-overlay").show();
        $.ajax({
            url: '{{route("cex.verifydocument", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id' : id,
                'status': status,
                'type' : statustype,
                'cex_hospital_bill_date': $("#bs-rangepicker-third").val(),
                'cex_admission_date': $("#bs-rangepicker-third").val(),
                'cex_discharge_date': $("#bs-rangepicker-forth").val(),
            },
            success: function (data) {
                $(".loader-overlay").hide();
                if(data.success) {
