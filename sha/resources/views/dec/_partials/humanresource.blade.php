                ...
                <div class=" mt-5 ">
                    <form id="ceoForm">
                        <table class="table table-responsive table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select class="select2 ceoerror" id="dec_statusceo" name="dec_status" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$hospital->ceo->dec_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$hospital->ceo->dec_status == "InValid") selected @endif >InValid</option>
                                            </select>
                                        </div>
                                    </td>
                                    <td>
                        ...
                        <div class="d-flex justify-content-end mt-3">
                            <button type="button" class="btn btn-primary mt-2 rounded-0 saveceoForm">Save</button>
                        </div>
                        @endif
                    </form>
                </div>
                            ...
                            <tbody class="table-border-bottom-0 procedure-body">
                                @foreach($hospital->humanResources()->where('type', 'mhr')->get() as $key => $value)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="form-floating form-floating-outline">
                                                <select disabled class="select2 mhrerror" id="dec_mhr_verify_status{{@$value->id}}" name="dec_verify_status_{{@$value->id}}" required>
                                                    <option value="">Select</option>
                                        ...
                                        <td>
                                            <div class="form-floating form-floating-outline">
                                                <select class="select2 mhrerror" id="dec_statushumanresource_{{@$value->id}}" name="dec_status_{{@$value->id}}" required>
                                                    <option value="">Select</option>
                        ...
                        <tbody class="table-border-bottom-0 procedure-body">
                            @foreach($hospital->humanResources()->where('type', 'sshr')->get() as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select disabled class="select2 sshrerror" id="dec_verify_status_sshrr{{@$value->id}}" name="dec_verify_status_{{@$value->id}}" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$value->dec_verify_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$value->dec_verify_status == "InValid") selected @endif >InValid</option>
                                            </select>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select class="select2 sshrerror" id="dec_statussupportservice_{{@$value->id}}" name="dec_status_{{@$value->id}}" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$value->dec_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$value->dec_status == "InValid") selected @endif >InValid</option>
                                            </select>
                        ...
                        <tbody class="table-border-bottom-0 procedure-body">
                            @foreach($hospital->hospitalTeam as $key => $value)
                                <tr id="hrrow{{$value->id}}">
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select disabled class="select2 specerror" id="dec_verify_status{{@$value->id}}" name="dec_verify_status_{{@$value->id}}" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$value->dec_verify_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$value->dec_verify_status == "InValid") selected @endif >InValid</option>
                                            </select>
                                    ...
                                    <td>
                                        <div class="form-floating form-floating-outline">
                                            <select class="select2 specerror" id="dec_statusspeciality_{{@$value->id}}" name="dec_status_{{@$value->id}}" required>
                                                <option value="">Select</option>
                                                <option value="Valid" @if(@$value->dec_status == "Valid") selected @endif >Valid</option>
                                                <option value="InValid" @if(@$value->dec_status == "InValid") selected @endif >InValid</option>
                                            </select>
...
<script>
    $('.saveceoForm').on('click', function() {        
        ...
        $.ajax({
            url: '{{route("dec.saveCEOReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
    ...
    $('.savemhrForm').on('click', function() {        
        ...
        $.ajax({
            url: '{{route("dec.saveMHRReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
    ...
    $('.savesshrForm').on('click', function() {        
        ...
        $.ajax({
            url: '{{route("dec.saveSSHRReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
    ...
    $('.savespecForm').on('click', function() {
        ...
        $.ajax({
            url: '{{route("dec.saveSPECReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
