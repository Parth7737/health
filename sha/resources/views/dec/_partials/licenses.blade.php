         ...
         <tbody
            ...
            @foreach($value->licenseType as $k => $v)
               ...
               <tr>
                  ...
                  <td>
                     <select class="select2 aerror" id="dec_status_{{$existData->id}}license" name="dec_status_{{$existData->id}}" required>
                           <option value="">Select</option>
...
<script>
   $('.savelicense').click(function () {
      ...
      $.ajax({
         url: '{{route("dec.saveLicensesReview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}', // Replace with your server endpoint
         headers: {
