            ...
            <div class="bs-stepper-content">        
                        ...
                        <div class="step @if(@$verification->status == 'Physical Verification Completed' && @$hospital->hospitalReport->dec_action == '') active @elseif(@$hospital->hospitalReport->dec_action != '') completed disabled crossed @endif" data-target="#social-links">
                            <button type="button" class="step-trigger">
                                ...
                                <span class="d-flex flex-column gap-1 ms-2">
                                    ...
                                    @if(@$hospital->hospitalReport->dec_action != ''  && @$hospital->hospitalReport->sec_action == '') <span class="bs-stepper-subtitle">({{$hospital->status}})</span>@endif
                                </span>
                                </span>
                            </button>
                        </div>
                ...
                <div class="card mb-6 ps-0 border border-primary">
                    <div class="card-body">
                        ...
                        @if($hospital->status == "Empanelment Not Recommended by DEC")<h5 class="text-center text-danger"><strong>{{$hospital->status}} By @if($hospital->dec_qry_id) {{\App\Models\User::where('id', $hospital->dec_qry_id)->first()->name}} @endif </strong></h5> @endif
                    </div>
                    <div class="d-flex justify-content-end mt-3 mb-2 me-3">
                        ...
                        @if(@$verification)
                            @if(@$verification->status == "Physical Verification Pending")
                                ...
                                <label for="" class="text-primary text-center me-2">Physical Verification Pending</label>
                            @elseif(@$verification->status == "Physical Verification Completed")
                                <button class="btn btn-primary rounded-0 me-3" id="preview">EmpanelMent Form</button>
                                <label for="" class="text-primary text-center me-2">Physical Verification Completed</label>
                            @endif
                           
                           
                        @else
                            <button class="btn btn-primary rounded-0 me-3" id="preview">EmpanelMent Form</button>
                            <a class="btn btn-outline-primary rounded-0" href="{{route('dec.initiate.verification', [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}">INITIATE PHYSICAL VERIFICATION</a>
                        @endif
                        @php($hosp = App\Models\Hospitals::where('id', @$hospital->main_hospitalid)->first())
                        @if(@$hosp && $hosp->is_upgrade_application == 1)                            
                            <a class="btn btn-outline-primary rounded-0" href="{{route('dec.initiate.verification', [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}">INITIATE PHYSICAL VERIFICATION</a>
                        @endif
                    </div>
                </div>
                @if(!@$verification && ($hospital->status == "Submitted" || $hospital->status == "Re-Submitted" || $hospital->status == "Upgradation Request" || $hospital->status == "Response Required From Facility" || $hospital->status == "Query On Upgradation Request From Facility"))
                            ...
                            <form id="hospitalReportFormDec" enctype="multipart/form-data">               
                                <div class="row g-5">  
                                    <div class="col-sm-4">
                                        ...
                                        <select name="dec_action" id="dec_action" class="select2 form-select form-select-lg decaction reporterrormesage" data-allow-clear="true" required >
                                    ...
                                    <div class="col-sm-4">
                                        <label class="mb-3">Upload Physical Verification Document <span class="text-danger">*</span></label>
                                        <div class="file-upload-section reporterrormesage">
                                            ...
                                            <input type="file" class="file-input d-none decattachment"  name="dec_document" id="dec_document"/>
                ...
                @endif
                @if(@$verification && $hospital->hospitalReport && @$hospital->status != 'Empanelment Not Recommended by DEC')
                <div class="bs-stepper-content">
                                                ...
                                                <ul class="nav nav-tabs ct-tabs" role="tablist">
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="true" onclick="loadStep(1);">
                                                            Basic Information  
                                                            @if(Helpers::stepCheck(1, $hospital->id, 'establishment_details', 'dec') && Helpers::stepCheck(1, $hospital->id, 'address', 'dec'))
                                                                @php($step1img = true)
                                                            @else
                                                                @php($step1img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false"  onclick="loadStep(2);">
                                                            Speciality
                                                            @if(Helpers::stepCheck(2, $hospital->id, 'speciality', 'dec'))
                                                                @php($step2img = true)
                                                            @else
                                                                @php($step2img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false" onclick="loadStep(3);">
                                                            Services    
                                                            @if(Helpers::stepCheck(3, $hospital->id, 'services', 'dec'))
                                                                @php($step3img = true)
                                                            @else
                                                                @php($step3img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false" onclick="loadStep(4);">
                                                            Statutory Licences   
                                                            @if(Helpers::stepCheck(4, $hospital->id, 'statutory_licences', 'dec'))
                                                                @php($step4img = true)
                                                            @else
                                                                @php($step4img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false" onclick="loadStep(5);">
                                                            Human Resources 
                                                            @if(Helpers::stepCheck(5, $hospital->id, 'ceo', 'dec') && Helpers::stepCheck(5, $hospital->id, 'mhr', 'dec') && Helpers::stepCheck(5, $hospital->id, 'sshr', 'dec') && Helpers::stepCheck(5, $hospital->id, 'specialist', 'dec'))
                                                                @php($step5img = true)
                                                            @else
                                                                @php($step5img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false" onclick="loadStep(6);">
                                                            ...
                                                            @if(Helpers::stepCheck(6, $hospital->id, 'quality_accreditation', 'dec'))
                                                                @php($step6img = true)
                                                            @else
                                                                @php($step6img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false" onclick="loadStep(7);">
                                                            ...
                                                            @if(Helpers::stepCheck(7, $hospital->id, 'finance_details', 'dec') && Helpers::stepCheck(7, $hospital->id, 'tax_details', 'dec'))
                                                                @php($step7img = true)
                                                            @else
                                                                @php($step7img = false)
                                                            @endif
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        aria-selected="false" onclick="loadStep(8);">
                                                            ...
                                                            @if(Helpers::stepCheck(8, $hospital->id, 'report', 'dec'))
                                                                @php($step8img = true)
                                                            @else
                                                                @php($step8img = false)
                                                            @endif
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                @if($hospital->status != 'Empannelled')
                    <!-- <button type="button" class="btn btn-primary previewsubmit" id="validate">Submit</button> -->
                @endif
            </div>
        </div>
    </div>
...
@push('scripts')
    <script>
        @if(@$verification && $hospital->hospitalReport)
        ...
        function loadStep(step) {
            ...
            if(step != ""){
                $.ajax({
                    url: '{{route("dec.stepLoad", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}', 
                    type: 'POST',
                    data: {
                        '_token': '{{csrf_token()}}',
                        'step' : step
        ...
        $("#preview").on("click",function(){
            ldrshow();
            $.ajax({
                url: '{{route("dec.hospital.preview", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
                headers: {
        ...
        $(document).ready(function () {
            let table = $('#workFlowList').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('dec.getWorkFlowData', [base64_encode($hospital->id), base64_encode($hospital->uuid)]) }}",
                    type: "POST",
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
        ...
        $('.saveHospitalReportFormDec').click(function () {
            swal({
                ...
                if (willDelete) {                    
                    ...
                    $.ajax({
                        url: '{{route("dec.submitDecResponse", [base64_encode($hospital->id), base64_encode($hospital->uuid)])}}',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        },
                        type: 'POST',
