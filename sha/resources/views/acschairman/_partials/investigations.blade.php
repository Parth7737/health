@php 
    $status = json_decode($preauth_register->acs_status, true);
@endphp
<style>
    .violate {
        color: #757bff;
    ...
    <div class="col-md-3">
        <table class="table">
            ...
            @foreach($preauth_register->investigations as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="status{{$value->id}}">@if($value->acs_status != "") @if($value->acs_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
            ...
            @foreach($preauth_register->enhancement_docs as $key => $value)
                <tr class="" style="text-decoration: none; display: -webkit-inline-box">
                    <td class="enhancement-status{{$value->id}}">@if($value->acs_status != "") @if($value->acs_status == "Correct") <i class="ri-file-check-fill text-primary"></i> @else <i class="ri-file-close-fill text-danger"></i> @endif @else <i class="ri-file-info-line"></i>  @endif</td>
                    <td style="padding:0;">
...
var documentsQueue = [];
    ...
    function clickondocument(id, type = '') {
        ...
        $.ajax({
            url: '{{route("acschairman.loadpdf", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id' : id,
    ...
    function verifyDocument(status, id, statustype = '') {
        $(".loader-overlay").show();
        $.ajax({
            url: '{{route("acschairman.verifydocument", [$preauth_register_id])}}', 
            type: 'POST',
            data: {
                '_token': '{{csrf_token()}}',
                'id' : id,
