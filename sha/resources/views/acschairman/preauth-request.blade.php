@extends('layouts.acschairman.app')
...
@php
    ...
    $helpers = new Helpers();
    $medicalinfo = $helpers->checkStepSeen($preauth_register->id, 'Medical_Information', 'ACS');
    $admissioninfo = $helpers->checkStepSeen($preauth_register->id, 'Admission_Informations', 'ACS');
    $treatmentinfo = $helpers->checkStepSeen($preauth_register->id, 'Treatment', 'ACS');
    $documentsinfo = $helpers->checkDocStepSeen($preauth_register->id, 'acs_status');
    $financeinfo = $helpers->checkStepSeen($preauth_register->id, 'Finance', 'ACS');
@endphp
    ...
    <div class="row">
        <div class="bs-stepper-content">
            <div class="card mb-6 ps-0 border border-primary">
                <div class="card-body">
                    <div class="row row-cols-5">
                        ...
                        <div class="col">
                            <div class="infodata">
                                <label>Preauth Initiated Amount</label>
                                <p><strong>₹{{ number_format($preauth_register->preauth_initiated_amount,2) }}</strong></p>
                            </div>
                            @if(@$preauth_register->committee_approved_amount)
                                <div class="infodata">
                                    <label>Committee Approved Amount</label>
                                    <p><strong>₹{{ number_format($preauth_register->committee_approved_amount,2) }}</strong></p>
                                </div>
                            @endif
                            @if(@$preauth_register->ceo_approved_amount)
                                <div class="infodata">
                                    <label>CEO Approved Amount</label>
                                    <p><strong>₹{{ number_format($preauth_register->ceo_approved_amount,2) }}</strong></p>
                                </div>
                            @endif
                            @if(@$preauth_register->acs_approved_amount)
                                <div class="infodata">
                                    <label>Preauth Approved Amount</label>
                                    <p><strong>₹{{ number_format($preauth_register->acs_approved_amount,2) }}</strong></p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="bs-stepper wizard-numbered mt-2">
                @include('acschairman._partials.preauth-step')
                <div class="bs-stepper-content">
                    <!-- Account Details -->
                    ...
                    <div id="personal-info" class="content active">
                                            ...
                                            <div class="col-12">
                                                <div class="table-responsive mt-5 text-nowrap">
                                                    <table class="table">
                                                        ...
                                                        <tbody>
                                                            @include('acschairman._partials.past-preauth')
                                                        </tbody>
                                                    </table>
                                                </div>
                                            </div>
                                                    ...
                                                    <div
                                                        ...
                                                        <table class="table">
                                                            ...
                                                            <tbody
                                                                ...
                                                                @include('acschairman._partials.diagnosis')
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div
                                                        ...
                                                        <table class="table">
                                                            ...
                                                            <tbody
                                                                ...
                                                                @include('acschairman._partials.procedures')
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="table-responsive mt-5 text-nowrap">
                                                    <table class="table">
                                                        ...
                                                        <tbody class="table-border-bottom-0 care-team-body">
                                                            @include('acschairman._partials.teams')
                                                        </tbody>
                                                    </table>
                                                </div>
                                            </div>
                                ...
                                <div id="accordionPopoutFive"
                                    ...
                                    <div class="accordion-body">
                                        <div class="inside-left-info-box mt-3">
                                            ...
                                            <div class="row justify-content-center">                                               
                                                @include('acschairman._partials.investigations',['preauth_register_id'=>$preauth_register->id])
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <div class="table-responsive mt-5 text-nowrap">
                                                    <table class="table">
                                                        ...
                                                        <tbody class="table-border-bottom-0 finance-body">
                                                            @include('acschairman._partials.finance')
                                                        </tbody>
                                                    </table>
                                                </div>
                                                <ul class="d-flex listing-right finance-total-body">
                                                    @include('acschairman._partials.finance-total')
                                                </ul>
                                            </div>
                                        </div>
                                        @if($preauth_register->status == PreauthRegister::STATUS_ACS_PENDING)
                                            <div class="action-form d-none mt-3">
                                                <form onSubmit="return false" id="approveRejectPreauthForm">
                                                    ...
                                                    <div class="col-md-3 pt-7 committee-approve-field d-none">
                                                        <div
                                                            class="form-floating form-floating-outline">
                                                            <input type="number" name="approved_amount" id="approved_amount" value="{{ $preauth_register->ceo_approved_amount }}" max="{{ $preauth_register->preauth_initiated_amount }}" class="form-control" />
                                                            <label for="approved_amount">Approve Amount</label>
                                                        </div>
                                                        <div id="committee-approve-amount-error"></div>
                                                    </div>
                                                    <div class="col-md-12 mt-2">
                                                        <div class="form-floating form-floating-outline mb-6 medical-committeeerror">
                                                    ...
                                                    @if($preauth_register->acs_query_response_remarks && $preauth_register->status == PreauthRegister::STATUS_ACS_PENDING)
                                                        <div class="col-md-12 mt-2">
                                                            <div class="form-floating form-floating-outline mb-6">
                                                                <textarea class="form-control h-px-50 text-danger" disabled >{{$preauth_register->acs_query_response_remarks}}</textarea>
                        ...
                        <div class="d-flex justify-content-end mt-3">
                            @if($preauth_register->status == PreauthRegister::STATUS_ACS_PENDING)
                                <button type="button" id="approve-preauth-btn" class="btn btn-primary ms-2 action-btn" onclick="actionPreath()">Action</button>
                            @endif
                        </div>
                    </div>
                    <!-- Social Links -->
...
<script>
    ...
    function actionPreath(){
        ...
        $("#approveRejectPreauthForm")[0].reset();
    }
    $("#preauth-btn").on("click",function(){
        ...
        swal({
            ...
            if (willDelete) {
                ...
                $('.error').remove();
                $.ajax({
                    url: '{{route("acschairman.approve-preauth")}}',
                    headers: {
                    ...
                    success: function (response) {
                        ...
                        if(response.success){
                            ...
                            setTimeout(() => {
                                window.location.href="{{ route('acschairman.dashboard') }}";
                            }, 1000);
                        }else{
                        ...
                        if (xhr.status === 422) { 
                            ...
                            for (let field in errors) {
                                if($(`select[name="${field}"]`).length > 0){
                                    ...
                                    $(`[name="${field}"]`).closest('.medical-committeeerror').after(`<div class="error text-danger">${errors[field][0]}</div>`);
                                }
                            }
                        } else {
    ...
    function opentab(tab, id) {
        ...
        $.ajax({
            url: '{{route("acschairman.open-tabs")}}',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            type: 'POST',
    ...
    $("#preauth_status").on("change",function(){
        if($(this).val() == 'Approve'){
            $(".committee-approve-field").removeClass("d-none");
        }else{
            $(".committee-approve-field").addClass("d-none");
            $("#preauth-btn").attr('disabled',false);
        }
    })
    $("#approved_amount").on("input", function () {
        ...
        if (!isNaN(amount) && !isNaN(max) && amount > max) {
            $("#committee-approve-amount-error").html("<span class='text-danger'>Approve Amount cannot be more than "+max+" Raise Amount.</span>");
            $("#preauth-btn").attr('disabled',true);
        } else {
            $("#committee-approve-amount-error").html("");
            if(amount != ''){
                $("#preauth-btn").attr('disabled',false);
            }else{
                $("#preauth-btn").attr('disabled',true);
