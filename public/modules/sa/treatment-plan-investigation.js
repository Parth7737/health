$(document).ready(function() {
    ...
    const xintable = $('#xin-table').DataTable({
        ...
        lengthChange: true,
        scrollX: true,
        ajax: { url: loadroute, type: 'POST', data: function (d) { d._token = window.Laravel.csrfToken; } },
        columns: [
            ...
            { data: 'name', name: 'name' },
            { data: 'code', name: 'code' },
            { data: 'scheme_type_name', name: 'scheme_type_name', orderable: false, searchable: false },
            { data: 'type', name: 'type' },
    ...
    initTooltips();
    xintable.on('draw.dt responsive-display.dt column-visibility.dt', initTooltips);
    ...
    $(document).on('click', '.adddata, .editdata', async function() {
        ...
        $.ajax({
            url: route('showform'), type: "POST", data: {id: id, _token: token},
            success: function (response) {
                loader('hide');
                if (response) { $("#ajaxdata").html(response); $(".add-datamodal").modal('show'); $(".add-datamodal .modal-dialog").removeClass('modal-xl'); loadSelect2(); }
            },
    ...
    $(document).on('click', '.deletebtn', async function() {
        var id = $(this).data('id');
        var url = route('destroy', {investigation : id});
        const token = await csrftoken();
        Swal.fire({
            ...
            if (result.isConfirmed) {
                loader();
                $.ajax({
                    url: url, type: "POST", data: {id: id, _token: token, _method: 'DELETE'},
                    success: function (response) {
                        loader('hide');
                        if (response.status) { sendmsg('success', response.message); $('#xin-table').DataTable().ajax.reload(null, false); }
                        else { sendmsg('error', response.message); }
                    }
                });
            }
        });
    ...
    $(document).on("submit", "#savedata", async function (e) {
        ...
        $.ajax({
            url: route('store'), type: "POST", data: fd, contentType: false, cache: false, processData: false,
            success: function (response) {
                loader('hide');
                if (response.status) { $(".add-datamodal").modal('hide'); $('#xin-table').DataTable().ajax.reload(null, false); sendmsg('success', response.message); }
                else { sendmsg('error', response.message); }
            },
            error: function (xhr) {
                loader('hide');
                ...
                if (xhr.status === 422) {
                    ...
                    for (let field in errors) {
                        ...
                        if ($field.hasClass('select2')) { $field.next('.select2-container').after(`<div class="err text-danger">${errors[field]['message']}</div>`); }
                        else { $field.after(`<div class="err text-danger">${errors[field]['message']}</div>`); }
                        errorMessages.push(errors[field]['message']);
                    }
                    if (errorMessages.length > 0) { sendmsg('error', errorMessages.join('<br>')); }
                } else { sendmsg('error', 'Something went wrong. Please try again later.'); }
            }
        });
    });
});
