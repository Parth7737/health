$(document).ready(function() {
    ...
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'visitor_purpose.name', name: 'purpose' },
            { data: 'name', name: 'name' },
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response) {
                    ...
                    flatpickr('input[type="date"]', { dateFormat: 'd-m-Y',maxDate: "today" });
                }
            },
            error: function (xhr) {
                loader('hide');
    ...
    $(document).on('click', '.deletebtn', async function() {
        var id = $(this).data('id');    
        var url = route('destroy', {visitor : id});
        const token = await csrftoken();
