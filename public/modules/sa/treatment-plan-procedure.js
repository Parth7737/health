$(document).ready(function() {
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'display_name', name: 'procedure_name', orderable: false, searchable: false },
            { data: 'procedure_code_1', name: 'procedure_code_1' },
    ...
    initTooltips();
    xintable.on('draw.dt', initTooltips);
    ...
    $(document).on('click', '.adddata, .editdata', async function() {
        ...
        $.ajax({
            ...
            success: function (response) {
                loader('hide');
                if (response) { $("#ajaxdata").html(response); $(".add-datamodal").modal('show'); $(".add-datamodal .modal-dialog").addClass('modal-xl'); loadSelect2(); }
            },
    ...
    $(document).on('click', '.deletebtn', async function() {
        ...
        Swal.fire({ title: 'Are you sure?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!' }).then((result) => {
            if (result.isConfirmed) {
                loader();
                $.ajax({
                    url: route('destroy', {procedure: id}), type: "POST", data: {_token: token, _method: 'DELETE'},
                    success: function (response) {
                        loader('hide');
