$(document).ready(function() {
    ...
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'unit_incharge_name', name: 'unit_incharge_name' },
            { data: 'is_video_consultation', name: 'is_video_consultation' },
    ...
    $(document).on('click', '.deletebtn', async function() {
        var id = $(this).data('id');    
        var url = route('destroy', {department_unit : id});
        const token = await csrftoken();
        Swal.fire({
            title: 'Are you sure?',
            ...
            error: function (xhr) {
                ...
                if (xhr.status === 422) { 
                    ...
                    for (let field in errors) {
                        ...
                        if ($field.hasClass('select2')) {/* Lines 141-142 omitted */} else {/* Lines 143-144 omitted */}
