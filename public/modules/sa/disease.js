$(document).ready(function() {
    ...
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'disease_type.name', name: 'diseaseType.name', defaultContent: '-' },
            { data: 'name', name: 'name' },
    ...
    $(document).on('click', '.deletebtn', async function() {
        var id = $(this).data('id');    
        var url = route('destroy', {disease : id});
        const token = await csrftoken(); // wait for the new token
        Swal.fire({
            title: 'Are you sure?',
    ...
    $(document).on("input", "#name", async function (e) {
        let name = $(this).val().trim();
        let code = name.split(/\s+/) // split by whitespace
