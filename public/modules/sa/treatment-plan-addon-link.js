$(document).ready(function() {
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'base_name', name: 'base_name', orderable: false, searchable: false },
            { data: 'addon_name', name: 'addon_name', orderable: false, searchable: false },
    ...
    $(document).on('click', '.deletebtn', async function() {
        ...
        Swal.fire({ title: 'Are you sure?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!' }).then((result) => {
            if (result.isConfirmed) {
                loader();
                $.ajax({
                    url: route('destroy', {addon_link: id}), type: "POST", data: {_token: token, _method: 'DELETE'},
                    success: function (response) {
                        loader('hide');
