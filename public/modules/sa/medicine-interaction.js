$(document).ready(function() {
    ...
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'interact_medicine_name', name: 'interactMedicine.name' },
            { data: 'severity_badge', name: 'severity' },
            { 
                data: 'clinical_effect', 
                name: 'clinical_effect',
                render: function(data, type, row) {
                    return data ? (data.length > 50 ? data.substr(0, 50) + '...' : data) : 'None';
                }
            },
            { 
                data: 'recommendation', 
    ...
    $(document).on('click', '.deletebtn', async function() {
        var id = $(this).data('id');    
        var url = route('destroy', {drug_interaction : id});
        const token = await csrftoken();
        Swal.fire({
            title: 'Are you sure?',
            ...
            error: function (xhr) {
                ...
                if (xhr.status === 422) { 
                    ...
                    for (let field in errors) {
                        ...
                        if ($field.hasClass('select2-modal') || $field.hasClass('select2')) {
