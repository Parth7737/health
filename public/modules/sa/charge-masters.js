$(document).ready(function() {
    ...
    const xintable = $('#xin-table').DataTable({
        ...
        columns: [
            ...
            { data: 'related_to', name: 'related_to', orderable: false, searchable: false },
    ...
    $(document).on('click', '.deletebtn', async function() {
        var id = $(this).data('id');
        var url = route('destroy', {charge_master: id});
        const token = await csrftoken();
    ...
    $(document).on('click', '#apply-all-tpa-rates', function () {
        const standardRate = $('#standard_rate').val();
        if (standardRate === '') {
            sendmsg('error', 'Please enter standard rate first.');
            return;
        }

        $('.tpa-rate-input').val(standardRate);
    });
